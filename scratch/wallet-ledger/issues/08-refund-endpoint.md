...
- Implemented `POST /players/{playerId}/wallet/refund` on `dev.wallet.api.WalletController` (a
  ...
  reason, Long originalDebitId)` record.
- **One-shot semantics** are enforced in the write transaction, which first takes the pessimistic write lock on
  ...
  2. **Already refunded** — if any entry already references the same `originalDebitId`, throw `409 already_refunded`.
  3. **Valid original debit** — the `originalDebitId` must exist and be a `DEBIT` entry, else `404 debit_not_found`.
  Then it appends a single `CREDIT` Ledger Entry (with `originalDebitId` set and `ReasonKind.REFUND`) and credits
  the Wallet in the same transaction. Because a Refund is a Credit, it is **never** subject to the Overdraft Guard.
- The DB UNIQUE constraints on `request_id` and `original_debit_id` (V1 schema) remain the final safety net
  behind the application checks, which is what makes the Duplicate Submission and one-shot rules hold under
  concurrency.
- Unknown player → `404 player_not_found`; missing or non-positive `originalDebitId` → `400`; missing/invalid
  body fields → `400` via the shared `require*` helpers. The controller reuses one `parse(...)` helper across
  credit/debit/refund (the refund call passes the extra `originalDebitId` on the side), so the amount/reason
...
- Tests: `src/test/java/dev/wallet/api/WalletRefundControllerTest.java` (8 tests) at the spec's single seam
  ...
  single refund entry), unknown player → `404`, unknown original debit → `404`, missing `originalDebitId` → `400`.
